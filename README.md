...
├── tests/                      # 228 个离线用例(无需 LLM / 网络 / 外部服务)
└── .github/workflows/          # touchstone.yml · calibrate.yml · govern.yml
```

生产代码约 3427 行 / 17 个模块;测试 228 个用例 / 12 个文件,全绿、离线;行覆盖率 83%(核心逻辑模块 85–100%;GitHub API / 子进程 / LLM / CLI 等集成层经 mock 覆盖)。
