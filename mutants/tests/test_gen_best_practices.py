...
# ---------------- generate / main ----------------
def test_generate_from_real_standards():
    md = G.generate(_STD, org="Touchstone")
    assert md.startswith("#") and "best practice" in md.lower()


def test_main_writes_to_file(tmp_path, monkeypatch):
    out = tmp_path / "bp.md"
    monkeypatch.setattr("sys.argv", ["gen_best_practices", "--standards", _STD, "--out", str(out)])
    G.main()
    assert out.read_text(encoding="utf-8").startswith("#")


def test_main_stdout(monkeypatch, capsys):
    monkeypatch.setattr("sys.argv", ["gen_best_practices", "--standards", _STD, "--out", "-"])
    G.main()
