...
def _ack_skill_ref():
    ...
    要点速览（无网 agent 至少从评论本身拿到最易错的规则；与 SKILL.md §2/§3/§4 同源，
    改口径须两处同步——test_skill_doc_sync 有防漂移断言）。行间用 <br>：本段落在
    HTML block 里，裸 \n 浏览器不换行（#192 教训）。

    【不贴正本正文】（#192 二级折叠后用户仍反馈混乱，拍板只留链接）：速览控制在
    5 行内——每行一个决策点，全文以链接为准；渲染零文件依赖。"""
    return (f"Agent 销项完整规程（可安装为 skill 或直接参考）：{_ACK_SKILL_URL}<br>\n"
            ...
            "③ 时序：改码 → 提交 → 发 ack 评论 → 推送（复检只读启动前已存在的申报，"
            "推送后补 ack 本轮不计；纯 ack 轮用空提交承载）；<br>\n"
            "④ 修复须落进测试套件；同根因发现逐条列出并注明同源。")
