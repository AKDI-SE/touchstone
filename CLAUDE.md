...
- [ ] 没有新增依赖（除非任务明确要求并已说明）；仓库须保持**离线可跑**（测试不依赖 LLM / 网络 / 外部服务）。
...
- **依赖克制。** 现有依赖只有 `unidiff` / `requests` / `openai` / `pyyaml`。PR-Agent 在**独立 venv、不进仓库**，只经 `pr_agent_runner.py` 子进程调用——不要把它的包塞进本仓依赖。
- **遇到这些信号立刻停下问人**：要删东西、要改契约/共有文件、要加依赖、要碰安全/凭据相关、跨层改动、或你发现自己在"为了让测试过而修改测试"。
